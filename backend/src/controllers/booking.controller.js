...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const application = await Application.findById(applicationId)
      ...
      .populate({ path: "presenter", select: "user", populate: { path: "user", select: "email name" } });
    ...
    // Check for an existing booking BEFORE the status check. Once a booking
    // is created, the application flips to "booked" — so on a repeat
    // attempt, checking status first would incorrectly report the generic
    // "must be shortlisted" (400) instead of the more accurate and specific
    // "a booking already exists" (409).
    ...
    return res.status(201).json(new ApiResponse(201, booking, "Booking created — proceed to payment from Bookings"));
  } catch (error) {
    next(error);
  }
};
...
export const completeBooking = async (req, res, next) => {
  try {
    ...
    if (booking.status !== "confirmed") {
      return next(
        new ApiError(400, `Booking must be "confirmed" (paid) before it can be marked completed. Current status: "${booking.status}"`)
      );
    }
    ...
    if (presenterProfile?.user) {
      await createNotification(req.app.get("io"), {
        ...
        message: "Your presentation has been marked complete and your payout has been released. Don't forget to leave a review!",
        meta: { bookingId: booking._id },
...
export const cancelBooking = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, booking, payment ? "Booking cancelled and payment refunded" : "Booking cancelled"));
  } catch (error) {
    next(error);
  }
};